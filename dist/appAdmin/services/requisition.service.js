    ...
    updateRequisition(req) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.db.transaction((trx) => __awaiter(this, void 0, void 0, function* () {
                ...
                const { organization_id, id } = req.admin;
                if (status === 'APPROVED') {
                    const checkApprover = yield model.checkApprover(organization_id, id, 'requisition_approvers');
                    if (checkApprover.length) {
                        ...
                        const userid = requisitionTrack.user_id;
                        const get_user_socket_id = yield this.Model.employeeModel().getSingleInfoEmployee(requisitionTrack.user_id);
                        ...
                        return {
                            ...
                            message: 'You Dont Have The Permission To Approve',
                        };
                    }
                }
                if (status === 'FULFILLED') {
                    const checkApprover = yield model.checkApprover(organization_id, id, 'requisition_fullfilers');
                    if (checkApprover.length) {
                        ...
                        const notify = yield this.Model.commonModel().createNotification({
                            type: 'requisition',
                            ref_id: requisition_id,
                            message: `Your Requisition  Has Been Delivered}`,
                        });
                        const employeeNotification = yield this.Model.commonModel().addEmployeeNotification({
                            notification_id: notify[0].id,
                            user_id: requisitionTrack.user_id,

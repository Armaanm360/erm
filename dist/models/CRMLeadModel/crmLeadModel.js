...
class CRMLeadModel extends schema_1.default {
    ...
    getAllLead(organization_id, { limit, skip, searchPrm, status, sec_status, third_status, team_id, emp_id, from_date, to_date, product_id, }) {
        return __awaiter(this, void 0, void 0, function* () {
            const dtbs = this.db("lead_view_v2");
            if (limit && skip) {
                dtbs.limit(parseInt(limit));
                dtbs.offset(parseInt(skip));
            }
    ...
    getSingleLead(id) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.db("lead_view_v2")
                .withSchema(this.CRM_SCHEMA)
                .select("*")
                .where("id", id);
        });
    ...
    getSingleLeadTracking_view(id) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.db("lead_tracking_view_v2")
                .withSchema(this.CRM_SCHEMA)
                .select("*")
                .where("lead_id", id);
        });
    ...
    getAllLeadTracking(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const dtbs = this.db("lead_tracking_view_v2");
            if (limit && skip) {
                dtbs.limit(parseInt(limit));
                dtbs.offset(parseInt(skip));
            }
            const data = yield dtbs
                .select("id", "lead_id", "org_id", "lead_org_id", "lead_org_name", "meeting_id", "follow_up_date", "title", "action_type", "estimated_amount", "sale_amount", "paid_amount", "due_amount", "description", "tracking_type", "remarks", "payment_collection_date", "created_at", "call_note", "forward_note", "emp_id", "emp_name", "admin_id", "admin_name")
                .withSchema(this.CRM_SCHEMA)
                .where({ org_id })
                .andWhere({ lead_id })
                .andWhere(function () {
                ...
                .orderBy("id", "asc");
            const total = yield this.db("lead_tracking_view_v2")
                .count("id as total")
                .withSchema(this.CRM_SCHEMA)
                .where({ org_id })
                .andWhere({ lead_id })
    ...
    (
        ...
        WHERE agr.assign_lead = e.id
            ...
            if (filter_by === "leave") {
                ...
                // if (from_date && to_date) {
                //   total = total.where('mv.meeting_date', '>=', formattedFromDate);
                //   total = total.where('mv.meeting_date', '<', formattedEndDate);
                // }
            }
            return {
                data: returnData,
...
`, [emp_id]);
    ...
    // add to history contact lead assign
    addHistoryContactLeadAssign(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.db("history_contact_lead_assign")
                .withSchema(this.CRM_SCHEMA)
                .insert(payload);
        });
    }

...
class EmployeeLeadModel extends schema_1.default {
  ...
  `, [assign_by])
                ...
                .andWhereRaw("CAST(contact_lead->>'follow_up_date' AS DATE) BETWEEN ? AND ?", [currentDate.toISOString(), endDate.toISOString()])
                .andWhere(function () {
                if (searchPrm) {
                ...
                .orderBy("id", "desc");
            const total = yield this.db("lead_view_v2")
                .withSchema(this.CRM_SCHEMA)
                .count("id as total")
                .where("organization_id", organization_id)
                .andWhereRaw("(contact_lead->>'status') = ?", ["positive_lead"])
  ...
  `, [assign_by])
    ...
    AllAfterSalesLeadList(organization_id, { limit, skip, searchPrm, assign_to, service_provided, sec_status, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const data = yield dtbs
                .withSchema(this.CRM_SCHEMA)
                .select("afs.id", "afs.lead_id", "afs.assign_to", "afs.follow_up_date", "afs.note as after_sale_note", "afs.paying_amount", "afs.type", "afs.targeted_amount", "afs.after_sale_lead_status", "afs.service_provided", "lv.contact_person", "lv.contact_email", "lv.org_name", "lv.reference", "tm.team_name", "pr.name as product_name", "sr.name as source_name", "ls.sale_amount", "ls.paid_amount", "ls.due_amount")
                .leftJoin("lead as lv", "lv.id", "=", "afs.lead_id")
    ...
    // insert history lead forward
    // public async insertHistoryLeadForward(payload: {
    //   lead_id: number;
    //   forward_to: number;
    //   forward_note?: string;
    // }) {
    //   await this.db("history_lead_forward")
    //     .withSchema(this.CRM_SCHEMA)
    //     .insert(payload);
    // }
    ...
    getEmployeeMonthlyTarget(organization_id, id, from_date, to_date) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.db("assign_monthly_target")
                .withSchema(this.CRM_SCHEMA)
                .select("phone_call", "visit", "sale", "sale_in_amount as sale_amount", "month")
    ...
    getEmployeeLeadCount(organization_id, id) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield this.db("lead")
                ...
                .where({ "contact_lead.assign_lead": id })
    ...
    getCurrentDayVisitStatistics(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const total = yield this.db("history_lead_visit as hlv")
                ...
                .join("lead as ld", "hlv.lead_id", "ld.id")

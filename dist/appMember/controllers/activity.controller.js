...
class MemberActivityController extends abstract_controller_1.default {
    constructor() {
        ...
        null, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _b = yield this.MemberActivityService.getPrayerTimes(req), { code } = _b, data = __rest(_b, ["code"]);
            res.status(code).json(data);
        }));
        //get team
        this.getActivities = this.asyncWrapper.wrap({
            ...
            const _c = yield this.MemberActivityService.getActivities(req), { code } = _c, data = __rest(_c, ["code"]);
            res.status(code).json(data);
        }));
        this.getMyActivity = this.asyncWrapper.wrap({
            ...
            const _d = yield this.MemberActivityService.getMyActivity(req), { code } = _d, data = __rest(_d, ["code"]);
            res.status(code).json(data);
        }));
        this.getMyActivityList = this.asyncWrapper.wrap({
            ...
            const _e = yield this.MemberActivityService.getMyActivityList(req), { code } = _e, data = __rest(_e, ["code"]);
            res.status(code).json(data);
        }));
        //update activity
        this.updateActivity = this.asyncWrapper.wrap(null, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _f = yield this.MemberActivityService.updateActivity(req), { code } = _f, data = __rest(_f, ["code"]);
            res.status(code).json(data);
        }));
        this.updateActivityEmployee = this.asyncWrapper.wrap(null, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _g = yield this.MemberActivityService.updateActivityByEmployee(req), { code } = _g, data = __rest(_g, ["code"]);
            res.status(code).json(data);
        }));
    }
}
